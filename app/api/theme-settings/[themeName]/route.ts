...
// ✅ GET corrigido (sem usar context.params)
export async function GET(request: NextRequest) {
    await dbConnect(); // Conecta ao banco de dados

    const pathParts = request.nextUrl.pathname.split('/');
    const themeName = decodeURIComponent(pathParts[pathParts.length - 1]); // Extrai themeName da URL
...
// ✅ POST continua funcionando com context.params
